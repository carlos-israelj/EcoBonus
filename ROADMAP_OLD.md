...
### ⚡ DECISIÓN ESTRATÉGICA: Trustless Work Integration

**Fecha**: 2026-09-23

**Decisión**: Reemplazar el RewardContract custom con **Trustless Work** escrow profesional para:
- ✅ Real XLM custody (blockchain-enforced, no solo accounting)
- ✅ Reducir desarrollo de 2+ semanas a 3 días
- ✅ Infraestructura profesional con dispute resolution
- ✅ AI automation support (backend auto-approval)
- ✅ Platform fee automation (4%)
- ✅ Transparency via Escrow Viewer

**Approach**: React SDK (Frontend) + REST API (Backend para AI validation)

**Documentación**: Ver `TRUSTLESS_WORK_FINAL_DECISION.md` para detalles completos

---

### 1. Trustless Work Integration 🚀 **[NUEVO - PRIORIDAD MÁXIMA]**

#### 1.1 Setup Inicial (Día 1 - Mañana)
**Tiempo estimado**: 4 horas

**Tareas**:
- [ ] Obtener API key de Trustless Work:
  - Ir a https://dapp.trustlesswork.com
  - Conectar wallet con Freighter
  - Generar testnet API key
...
- [ ] Crear tipos TypeScript:
  - `src/types/trustless-work.ts`
  - Interfaces para payloads
  - Role mappings

**Entregables**:
- Frontend configurado con Trustless Work SDK
- API key funcionando
- Provider wrapping la app

---

#### 1.2 Frontend Implementation (Día 1 - Tarde)
**Tiempo estimado**: 4 horas

**Tareas**:
- [ ] Crear `src/hooks/useMissionEscrow.ts`:
  ```typescript
  import {
    useInitializeEscrow,
    useFundEscrow,
  ...
  export const useMissionEscrow = () => {
    // Wrapper para todas las operaciones de escrow
    // createMission, fundMission, submitClaim, distributeReward
  }
  ```
- [ ] Actualizar componentes:
  - [ ] `MissionCard.tsx` → Mostrar escrow status
  - [ ] `CreateMission.tsx` → Deploy escrow + fund
  - [ ] `ClaimReward.tsx` → Change milestone status
  - [ ] `AdminDashboard.tsx` → Release funds flow
- [ ] Agregar React Query para polling:
  ```typescript
  const { data: escrow } = useQuery({
    queryKey: ["escrow", escrowId],
    queryFn: () => getEscrowsByContractIds([escrowId]),
    refetchInterval: 5000
  });
  ```
- [ ] UI loading states y error handling

**Entregables**:
- Hooks funcionales para escrow lifecycle
- Componentes actualizados con Trustless Work
- Real-time escrow status polling

---

#### 1.3 Backend AI Validation (Día 2)
**Tiempo estimado**: 8 horas

**Tareas**:
- [ ] Crear API routes en `backend/src/routes/`:
  - [ ] `POST /api/validate-claim` - AI validation endpoint
  - [ ] `POST /api/approve-milestone` - Auto-approval after AI
  - [ ] `POST /api/release-funds` - Distribution endpoint
- [ ] Implementar AI validation service:
  ```typescript
  // backend/src/services/aiValidation.ts
  async function validateClaim(escrowId: string, evidenceUri: string) {
    // 1. Download image from IPFS
    // 2. Call AI service for validation
    // 3. If valid, call Trustless Work REST API
    // 4. Sign transaction with admin key
    // 5. Submit to blockchain
  }
  ```
- [ ] Integración con Trustless Work REST API:
  ```typescript
  const response = await axios.post(
    "https://dev.api.trustlesswork.com/escrow/multi-release/approve-milestone",
    {
      contractId: escrowId,
      milestoneIndex: "0",
      approver: ADMIN_ADDRESS
    },
    { headers: { "x-api-key": process.env.TRUSTLESS_WORK_API_KEY } }
  );
  ```
- [ ] Admin wallet signing:
  ```typescript
  import { Keypair } from "@stellar/stellar-sdk";
  const adminKeypair = Keypair.fromSecret(process.env.ADMIN_PRIVATE_KEY);
  const signedXdr = await adminKeypair.sign(unsignedTransaction);
  ```
- [ ] Webhook setup para trigger desde frontend
- [ ] Error handling y logging

**Entregables**:
- Backend endpoints funcionales
- AI validation integrado con auto-approval
- Admin signing working
- Logs y error handling

---

#### 1.4 Integration & Testing (Día 3)
**Tiempo estimado**: 8 horas

**Tareas**:
- [ ] End-to-end workflow testing:
  1. Sponsor crea misión (deploy escrow)
  2. Sponsor fondea escrow con XLM
  3. User descubre misión en mapa
  4. User completa misión y sube evidencia
  5. AI valida evidencia
  6. Backend auto-aprueba milestone
  7. Backend libera fondos
  8. User recibe XLM (menos 4% platform fee)
  9. CertificateNFT minted
- [ ] Conectar frontend → backend webhooks:
  ```typescript
  // Después de submitClaim
  await fetch("/api/validate-claim", {
    method: "POST",
    body: JSON.stringify({ escrowId, evidenceUri })
  });
  ```
- [ ] Testing edge cases:
  - [ ] Invalid evidence (AI rejects)
  - [ ] Insufficient escrow funds
  - [ ] Duplicate claims
  - [ ] Expired missions
- [ ] UI polish:
  - [ ] Loading states durante AI validation
  - [ ] Success/error notifications
  - [ ] Escrow status badges
  - [ ] Transaction confirmations
- [ ] Verificar en Escrow Viewer:
...
**Entregables**:
- Flujo completo funcionando en testnet
- Edge cases manejados
- UI pulida con feedback states
...
#### 2.1 MissionContract - Mantener ✅
**Estado**: Mantener como está

El MissionContract sigue siendo necesario para:
- Metadata de misiones (título, descripción, ubicación)
- Link al escrow de Trustless Work (contractId)
- Validación GPS
- Estado de misión (activa, completada, expirada)

**No requiere cambios mayores** - solo agregar campo `escrow_id` para trackear el escrow.

---

#### 2.2 RewardContract - DEPRECAR ❌
**Estado**: Reemplazado por Trustless Work

El RewardContract custom será reemplazado completamente por Trustless Work escrow.

**Razón**:
- Custom contract solo hace accounting (no real custody)
- Trustless Work ofrece real blockchain-enforced escrow
- Reduce mantenimiento y desarrollo time

**Migración**: Ver `TRUSTLESS_WORK_INTEGRATION.md` sección "Migration Plan"

---

#### 2.3 CertificateNFT - Mantener ✅
**Estado**: Mantener como está

CertificateNFT sigue siendo único de EcoBonus:
- Minteo de achievement NFTs
- Marketplace B2B
- Carbon offset tracking
- Impact certificates

**Integración con Trustless Work**:
- Mintear NFT DESPUÉS de que Trustless Work release funds
- Usar webhook desde backend después de successful release
